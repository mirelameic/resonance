...
- [ ] **Step 2: Clear the placeholder dataset before syncing**

`js/data.js` currently holds Task 6's small placeholder (6 works with fake `source.sourceId` values like `"placeholder-1"`). The sync's merge logic (`mergeWorks`) only replaces an existing entry when a fresh entry shares its exact `source.type` + `source.sourceId` — the placeholders' fake ids will never match a real TMDB numeric id, so if left in place they would sit alongside the real synced works forever instead of being replaced. Reset the file to an empty dataset first so the real sync starts clean:
...
- [ ] **Step 7: Report real numbers to the user**
